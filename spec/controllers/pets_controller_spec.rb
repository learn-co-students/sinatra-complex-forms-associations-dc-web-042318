...
describe "Pets Controller" do
  describe "new action" do
    ...
    it "has a form with a checkbox for existing owners" do
      ...
      visit '/pets/new'
      expect(page.has_unchecked_field?("owner_#{@owner1.id}")).to eq(true)
      expect(page.has_unchecked_field?("owner_#{@owner2.id}")).to eq(true)
    end
  ...
  describe "edit action" do
    ...
    it " loads form to edit a pet and his owner" do
      ...
      expect(page).to have_field('pet_name')
      expect(page.has_checked_field?("owner_#{@owner.id}")).to eq(true)
